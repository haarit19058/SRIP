...
class NullInversion:
    ...
    def ddim_loop(self, latent):
        ...
        print('Storing text condition latent at each timesteps...')
        for i in tqdm(range(self.NUM_DDIM_STEPS)):
            t = self.model.scheduler.timesteps[len(self.model.scheduler.timesteps) - i - 1]
            noise_pred = self.get_noise_pred_single(latent, t, cond_embeddings) # Original Code
            # noise_pred = self.get_noise_pred_single(latent, t, uncond_embeddings) # Modified
    ...
    def invert(self, image_path, prompt, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
        self.init_prompt(prompt)
        attention_utils.register_attention_control(self.model, None)
